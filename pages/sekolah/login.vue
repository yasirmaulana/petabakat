  ...
  <div class="min-h-screen bg-gray-25 font-body flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
      <div class="mb-6 text-center">
        ...
        <h1 class="mt-4 text-2xl font-bold text-gray-950">Login Dashboard Sekolah</h1>
        <p class="mt-1 text-sm text-gray-500">Masuk dengan akun admin atau guru sekolah</p>
      </div>

      <form @submit.prevent="submit" class="card p-6 space-y-4">
        <div>
          <label class="label-text">Email</label>
          <input v-model="form.email" type="email" required placeholder="admin@sekolah.sch.id" class="input-field" />
        </div>
        <div>
      ...
      <p class="mt-4 text-center text-xs text-gray-400">
        Belum punya akun? <NuxtLink to="/sekolah/daftar" class="text-brand-600 hover:underline">Daftarkan sekolah</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
<script setup>
useSeoMeta({ title: 'Login Sekolah — PetaMinatBakat' })
