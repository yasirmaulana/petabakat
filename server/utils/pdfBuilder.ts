...
// Word-wrap using getTextWidth — accurate because font state matches rendering
...
export function buildPdfBuffer(result: any): Buffer {
  ...
  const W = 210
  const M = 20
  const CW = W - M * 2  // 170mm usable width
  ...
  color(doc, DARK); doc.setFontSize(9); doc.setFont('helvetica', 'normal')
  const descLines = wrap(doc, result.personaDescription || '', CW - 10)
  const boxH = 16 + descLines.length * 5
  ...
  color(doc, DARK); doc.setFontSize(9); doc.setFont('helvetica', 'normal')
  const narrLines = wrap(doc, result.scoreNarrative || '', CW)
  ...
  color(doc, GREEN); doc.setFontSize(13); doc.setFont('helvetica', 'bold')
  const planTitle = wrap(doc, result.microdosingPlan?.title || 'Rencana Stimulasi Mingguan', CW)
  doc.text(planTitle, M, y)
  stroke(doc, AMBER); doc.setLineWidth(0.8)
  doc.line(M, y + 2, M + 55, y + 2)
  y += (planTitle.length - 1) * 6 + 9

  for (const item of result.microdosingPlan?.schedule || []) {
    // compute actLines with correct font before page-break check
    doc.setFontSize(9); doc.setFont('helvetica', 'bold')
    const actLines = wrap(doc, item.activity || '', CW)
    const rowH = 20 + (actLines.length - 1) * 5
    if (y + rowH > 272) { doc.addPage(); y = 20 }

    // day badge: auto-width
